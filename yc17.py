"""
    Có bao nhiêu hóa đơn có thành tiền từ x đồng trở lên đã bán trong
tháng y năm 2023
"""
...
x = float(input('Nhập x (đồng): '))
y = input('Nhập tháng y (1-12): ')

print(f'Danh sách các hóa đơn có thành tiền từ {x} đồng trở lên, bán trong tháng {y}/2023: ')
for info in data[:-1]:
    tmp = info.split()      #tmp = ['01:54:43', '27/09/2023', '1,12497,Pana,11,1,2,24000000.0,48000000.0']
    if float(tmp[2].split(',')[-1])>=x and tmp[2].split(',')[4]==y:
        print(info)