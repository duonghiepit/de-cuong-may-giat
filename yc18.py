"""
    Có bao nhiêu hóa đơn có số lượng từ x đến y
"""
...
x, y = map(int, input('Nhập số lượng x, y (cách nhau bởi dấu cách): ').split())
c = 0
for info in data[:-1]:
    tmp = info.split()      #tmp = ['01:54:43', '27/09/2023', '1,12497,Pana,11,1,2,24000000.0,48000000.0']
    if x<=int(tmp[2].split(',')[5])<=y:
        c += 1

print(f'Có {c} hóa đơn bán máy giặt có số lượng từ {x} đến {y}!')