"""
    Xác định Số hóa đơn và tên máy giặt có thành tiền nhỏ nhất đã bán
...
tt_min = min([float(element.split()[2].split(',')[-1]) for element in data[:-1] if element.split()[2].split(',')[4]==x])

for info in data[:-1]:
    tmp = info.split()      #tmp = ['01:54:43', '27/09/2023', '1,12497,Pana,11,1,2,24000000.0,48000000.0']
    if tmp[2].split(',')[-1]==f'{tt_min}' and tmp[2].split(',')[4]==x:
